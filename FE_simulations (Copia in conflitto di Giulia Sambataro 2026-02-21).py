...
plt.semilogy(lambda_sigma1/lambda_sigma1[0], 'o-',label=r'$\sigma=0.5$')
plt.semilogy(lambda_sigma2/lambda_sigma2[0], 's-',label=r'$\sigma=0.005$')
plt.title('POD Eigenvalue Decay',fontsize=20)
plt.xlabel('Mode Number',fontsize=20)
plt.ylabel('Normalized Eigenvalue (log scale)', fontsize=20)
#plt.ylabel(r'$\ell_k/\ell_1$',fontsize=15)
plt.grid(True)
plt.tight_layout()
plt.legend()
...
plt.figure(figsize=(8, 5))
plt.semilogy(range(1, ntrain + 1), e_k_sigma1, 'o-', label=fr'$\sigma = {sigma1}$')
plt.semilogy(range(1, ntrain + 1), e_k_sigma2, 's-', label=fr'$\sigma = {sigma2}$')
plt.xlabel("Number of POD Modes $k$")
plt.ylabel(r" square error $e_k = \frac{\sum_{j \geq k} \lambda_j}{\sum_j \lambda_j}$")
plt.title(" Square error Decay (POD Error) vs. Number of Modes")
